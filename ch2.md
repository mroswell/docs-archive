...
**Build up some data** We're going to add some dummy data in a new folder called `data`. Create that folder in `src` and add a blank file called `dogs.js`. Paste in the following json object:
...
**Refactor the template - create a Prop!** Create a new folder inside the `src` and name it `components`.
...
```
34. Now we have to pass a `dog` prop to our `Dog` component. It will be done with the familiar `v-bind` directive (remember, you can use its `:` shortcut). Edit the code you just added to `Pets.vue`:
